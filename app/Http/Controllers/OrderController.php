...
class OrderController extends Controller
{
    ...
    public function PastOrders()
    {
        ...
        // جلب الطلبات التي مضى عليها أكثر من 24 ساعة
        $pastOrders = Order::where('user_id', $user->id)
            ->where('created_at', '<', Carbon::now()->subHours(24)) // الطلبات التي مرّ عليها 24 ساعة أو أكثر
            ->orderBy('created_at', 'desc') // ترتيب الطلبات من الأحدث إلى الأقدم
            ->get();
    
        return response()->json([
            'message' => 'تم جلب الطلبات السابقة بنجاح',
